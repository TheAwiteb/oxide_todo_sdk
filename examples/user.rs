...
async fn main() {
    ...
    let registered_user = client.register("username", "password").await.unwrap();
    println!("Registered a new user successfully. User: {registered_user:?}");
    // login the user with username and password.
    println!("Logining the user...");
    let logined_user = client.login("username", "password").await.unwrap();
    ...
    let by_token_user = client.login_by_token("token").unwrap();
